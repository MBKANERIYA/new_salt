...
const Bracelet = () => {
  ...
  const getUploadData = async () => {
    ...
    try {
      ...
      const categoryProducts = response.data.filter(item => item.category === "Ladies Bracelet");
  ...
  const applyFilters = async (appliedFilters) => {
    ...
    try {
      ...
      const filteredProducts = response.data.updatedProducts.filter(item => item.category === "Ladies Bracelet");
  ...
  return (
    <Helmet title="Bracelets & Bangles">
