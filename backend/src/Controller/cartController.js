...
module.exports.addToCart = async (req, res) => {
    try {
        let { product, user, quantity, size, caratBy, colorBy } = req.body;
        ...
        // Only validate user in DB if it's a valid ObjectId (skip for guest UUIDs)
        ...
        if (cart) {
            ...
            if (productIndex > -1) {
                ...
                cart.quantity.push({ productId: product, quantity: quantity || 1, colorBy: colorBy || '', caratBy: caratBy || '', size: size || '' });
            }
            ...
            const newCart = await cartSchema.create({
                userId: user,
                quantity: [{ productId: product, quantity: quantity || 1, colorBy: colorBy || '', caratBy: caratBy || '', size: size || '' }]
            });
        ...
        const updatedCart = {
            ...
            quantity: cart.quantity
                ...
                .map(product => ({
                    ...
                    colorBy: product.colorBy || '',
                    caratBy: product.caratBy || '',
                    size: product.size || '',
                    totalPriceOfProduct: (product.quantity || 0) * (product.productId?.total14KT || 0)
                }))
        };
