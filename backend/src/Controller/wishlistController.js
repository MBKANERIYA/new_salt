...
module.exports.addToWishlist = async (req, res) => {
  ...
  try {
    // Check if the user exists (skip for guest UUIDs)
...
module.exports.getWishlist = async (req, res) => {
  ...
  try {
    ...
    // If no wishlist document exists (e.g. last item was removed and doc was deleted)
    if (!wishlist) {
      return res.status(200).send({ status: true, message: "Wishlist is empty", wishlist: { products: [] } });
    }
